...
IF NOT EXIST "%current_path%\bin\yt-dlp.exe" (
	...
	curl -L https://github.com/yt-dlp/yt-dlp/releases/latest/download/yt-dlp.exe --output "%current_path%\bin\yt-dlp.exe"
)

IF NOT EXIST "%current_path%\bin\ffmpeg.exe" (
	IF NOT EXIST "%current_path%\ffmpeg.zip" (
		ECHO Downloading FFmpeg
		curl -L https://github.com/BtbN/FFmpeg-Builds/releases/latest/download/ffmpeg-n8.1-latest-win64-gpl-8.1.zip --output "%current_path%\ffmpeg.zip"
	)
